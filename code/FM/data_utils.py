...
def map_features(user_feature_path, item_feature_path):
    ...
    for userID in user_feature:
        ...
        #         changes to test the performance without userID. because userid contains the info of historical interactions.
        mapped_user_feature[userID] = [mapped_feat, user_feature[userID][1]]
    #         mapped_user_feature[userID] = [mapped_feat[1:], user_feature[userID][1][1:]]
    #     print(mapped_user_feature)
    mapped_item_feature = {}
...
class FMData(data.Dataset):
    ...
    def __init__(self, train_path, user_feature, item_feature, loss_type, negative_num=1):
        ...
        for pair in self.train_list:
            ...
            self.pos_feature_values.append(np.array(self.user_feature[userID][1] + self.item_feature[itemID][1], dtype=np.float32))
    ...
    def ng_sample(self):
        ...
        for pair in self.train_list:
            ...
            for i in range(self.negative_num):
                ...
                neg_feature_values.append(
                    np.array(self.user_feature[userID][1] + self.item_feature[j][1], dtype=np.float32))
